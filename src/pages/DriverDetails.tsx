...
const DriverDetails = () => {
  ...
  const [activeTab, setActiveTab] = useState<'info' | 'gps' | 'wallet' | 'rides'>('info');
  ...
  return (
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column' }}>
      ...
      {/* Tabs Navigation */}
      <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1.5rem', borderBottom: '1px solid var(--border)', paddingBottom: '0.75rem', overflowX: 'auto', flexWrap: 'wrap' }}>
        <button
          className={`btn ${activeTab === 'info' ? 'btn-primary' : 'btn-outline'}`}
          onClick={() => setActiveTab('info')}
          style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', borderRadius: '10px', padding: '0.6rem 1.25rem', fontWeight: 600, fontSize: '0.92rem' }}
        >
          <FileText size={18} /> Info & Documents
          {driver.rawDocs && driver.rawDocs.length > 0 && (
            <span style={{ background: activeTab === 'info' ? 'rgba(255,255,255,0.25)' : 'var(--input-bg)', padding: '0.15rem 0.5rem', borderRadius: '12px', fontSize: '0.75rem', marginLeft: '0.25rem' }}>
              {driver.rawDocs.length}
            </span>
          )}
        </button>

        <button
          className={`btn ${activeTab === 'gps' ? 'btn-primary' : 'btn-outline'}`}
          onClick={() => setActiveTab('gps')}
          style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', borderRadius: '10px', padding: '0.6rem 1.25rem', fontWeight: 600, fontSize: '0.92rem' }}
        >
          <Navigation size={18} /> Live GPS & History
        </button>
        ...
        <button
          ...
          style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', borderRadius: '10px', padding: '0.6rem 1.25rem', fontWeight: 600, fontSize: '0.92rem' }}
        >
          <IndianRupee size={18} /> Earnings & Wallet
          {walletRecords.length > 0 && (
            <span style={{ background: activeTab === 'wallet' ? 'rgba(255,255,255,0.25)' : 'var(--input-bg)', padding: '0.15rem 0.5rem', borderRadius: '12px', fontSize: '0.75rem', marginLeft: '0.25rem' }}>
        ...
        <button
          className={`btn ${activeTab === 'rides' ? 'btn-primary' : 'btn-outline'}`}
          onClick={() => setActiveTab('rides')}
          style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', borderRadius: '10px', padding: '0.6rem 1.25rem', fontWeight: 600, fontSize: '0.92rem' }}
        >
          <Route size={18} /> Ride History
          {driverRides.length > 0 && (
            <span style={{ background: activeTab === 'rides' ? 'rgba(255,255,255,0.25)' : 'var(--input-bg)', padding: '0.15rem 0.5rem', borderRadius: '12px', fontSize: '0.75rem', marginLeft: '0.25rem' }}>
              {driverRides.length}
            </span>
          )}
        </button>
      ...
      {activeTab === 'info' && (
        <div className="glass-panel animate-fade-in" style={{ padding: '1.5rem', background: 'var(--bg-main)', border: '1px solid var(--glass-border)', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <h3 style={{ fontSize: '1.1rem', color: 'var(--accent-primary)', margin: 0 }}>Driver Documents</h3>
            {(!driver.rawDocs || driver.rawDocs.length === 0) ? (
      ...
      {/* TAB 2: Live GPS & History */}
      {activeTab === 'gps' && (
        <div className="glass-panel animate-fade-in" style={{ padding: '1.5rem', background: 'var(--bg-main)', border: '1px solid var(--glass-border)' }}>
